...
export function targetBudget({ targetSize, duration, copyAudio, useGPU }, info) {
  ...
  const safetyBytes = requestedBytes * (useGPU ? 0.06 : 0.02);
  ...
  return { requestedBytes, videoBitrate, audioBitrate, muxBytes, safetyBytes,
    ...
    approximate: Boolean(useGPU || (audio && copyAudio)) };
}
